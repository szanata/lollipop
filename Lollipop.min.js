...
Lollipop.config=function(b){$.extend(j,d,b)};Lollipop.open=function(c){var a={};$.extend(a,j,c);b.find("#lollipop-popup").css({maxWidth:a.maxWidth||"auto",maxHeight:a.maxHeight||"auto",minWidth:a.minWidth||"auto",minHeight:a.minHeight||"auto",width:a.width||"auto",height:a.height||"auto"});a.showHeader&&a.title?b.find("#lollipop-popup-header").show().html(a.title):b.find("#lollipop-popup-header").hide();b.find("#lollipop-popup-body").html(a.content);if(!a.showFooter||(!a.buttons||0===a.buttons.length)&&
!a.showCancelButton)b.find("#lollipop-popup-footer").empty().hide();else{b.find("#lollipop-popup-footer").empty().show();if(a.buttons)for(var c=0,d=a.buttons.length;c<d;c++)h(a.buttons[c].title,a.buttons[c].click);a.showCancelButton&&(h(a.cancelButtonTitle,function(){if("function"===typeof a.onCancel)a.onCancel();Lollipop.close()}),$(window).on("keydown.lollipop",function(b){if(27===b.keyCode){if("function"===typeof a.onCancel)a.onCancel();Lollipop.close()}}))}b.hide().appendTo($("body"));a.animate?
(b.css("visibility","hidden").show(),i(),b.hide().css("visibility","visible").fadeIn()):(b.show(),i())};Lollipop.close=function(c){c?b.remove():b.fadeOut(function(){b.remove()});$(window).off("keydown.lollipop");$(window).off("resize.lollipop");$(window).add("html, body").off("scroll.lollipop")};Object.freeze&&Object.freeze(Lollipop)})();
